...
@app.route('/')
def index():
    return "<link rel=\"shortcut icon\" href=\"/favicon.ico\"><title>Clipboard Generator</title><style>footer{font-family: \"Helvetica Neue\", Arial, sans-serif;position: absolute; left:0; right: 0; padding: 1rem; bottom: 0;background-color: #efefef;text-align: center;}</style><h1>Welcome to Clipboard clipboard generator by <a href=\"https://github.com/N3evin\">N3evin</a></h1><div><fieldset><legend>Generate Clipboard clipboard format data</legend><p><h2>/spreadsheet/{spreadsheet id}/{sheet name}/{row}</h2></p><ul><li><b>spreadsheet id:</b> id from spreadsheet url.</li><li><b>sheetname:</b> sheet name from spreadsheet to read from.</li><li><b>row:</b> which row of spreadsheet to retrieve information from.</li></ul></fieldset></div><footer>Copyright &copy; <script>new Date().getFullYear()>2010&&document.write(new Date().getFullYear());</script>, <a href=\"https://github.com/N3evin/pkmn-clipboard-spreadsheet\">N3evin</a></footer>"
...
def generateClipboard(key,sheet,row):
    ...
    species = jsonResult['Species'].title()
    ...
    elif (genderTextPokemon(species)):                 # with female/male sign
    ...
    elif(species.lower() == "basculin-blue"):           # For Basculin Blue
        speciesImage = "basculin-blue-striped"
...
# Check for gender text in these set of pokemon.
def genderTextPokemon(speciesName):
    pokemon = ["frillish", "jellicent", "pyroar", "meowstic"]; # Special named pokemon
    for name in pokemon:
        if(name in speciesName.lower()):
            return True
    return False;
